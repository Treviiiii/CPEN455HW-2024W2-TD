...
if __name__ == '__main__':
    ...
    model_name = 'conditional_pcnn_EF_withAccuracy_' + args.dataset + "_"
    ...
    if args.en_wandb:
        ...
        # USE FOR CONTINUING RUNS
        wandb.config.update({"current_time": time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}, allow_val_change=True)
    ...
    loss_op   = lambda real, fake : discretized_mix_logistic_loss(real, fake, reduction = 'mean')
    ...
    for epoch in tqdm(range(args.start_epoch, args.max_epochs)): #modified to 100 to continue training from 100 epochs until 150. 
        ...
        if "cpen455" in args.dataset: 
            ...
            print("Epoch {:d} - classification accuracy: {:.4f}".format(epoch, val_acc))
